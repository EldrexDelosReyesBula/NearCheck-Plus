    ...
    const state = {
        ...
        teacherDevices: new Map() // Store teacher Bluetooth devices
    };
    ...
    const NeuralNetworkEngine = {
        ...
        // Validate location with neural network simulation
        validateLocation: function(userLocation, sectionLocation, radius, userAccuracy = 10) {
            ...
            // Factor in location accuracy
            const accuracyFactor = Math.max(0, 1 - (userAccuracy / 50));

            // Factor in time of day (simulate patterns)
            const now = new Date();
            const hour = now.getHours();
            let timeFactor = 1;

            // Lower confidence during unusual hours
            if (hour < 7 || hour > 22) {
                timeFactor = 0.8;
            }
        ...
        detectFraudPatterns: function(userLocation, sectionLocation, confidence) {
            ...
            if (state.currentLocation && state.currentLocation.timestamp) {
                const timeDiff = (new Date() - state.currentLocation.timestamp) / 1000; // in seconds
                ...
                // If location changed too quickly (more than 100 m/s)
            ...
            for (const loc of commonSpoofingLocations) {
                ...
                if (distance < 1000) { // Within 1km of known spoofing location
                    return true;
                }
            }

            // Check for mock location apps (common altitude values)
        ...
        // Generate NearID for device identification
    ...
    // Enhanced Location Services with Web Bluetooth and Enhanced Precision
    const LocationService = {
        // Get current location with enhanced precision
        ...
        // Request location permission with WATT transparency
        ...
        // Continuous location monitoring for auto-checkin
    ...
    // Enhanced BLE Service for Bluetooth check-in
    const BLEService = {
        currentDevice: null,
        isScanning: false,
        teacherDevices: new Map(),

        // Register teacher Bluetooth device
        ...
        // Get teacher Bluetooth device for section
        ...
        // Scan for BLE devices with filtering
        ...
        // Validate BLE device against section
        validateBLEDevice: function(deviceId, sectionId) {
            return new Promise((resolve, reject) => {
                // Get the registered teacher device for this section
        ...
        // Register current device as teacher device
    ...
    const AutoCheckinService = {
        ...
        // Start auto check-in monitoring
        start: function() {
        ...
        // Check current location for auto check-in opportunities
        ...
        // Process location for auto check-in
        ...
        updateValidation: function(location) {
            ...
            if (!validation.valid || validation.fraudDetected) {
                // Location is no longer valid, cancel auto check-in
    ...
    function showApp() {
        ...
        if (state.userRole !== 'teacher') {
            LocationService.requestLocationPermission().catch(() => {
                // Permission denied, but we can still continue
                showToast('warning', 'Location Access', 'Some features may not work without location access');
            });
        }

        // Start auto check-in monitoring for students
    ...
    document.getElementById('registerForm').addEventListener('submit', (e) => {
        ...
        // Check if user selected teacher role and is under 20
    ...
    function setupEventListeners() {
        ...
        // Register Bluetooth device for teachers
        const registerBleDevice = document.getElementById('registerBleDevice');
        if (registerBleDevice) {
            registerBleDevice.addEventListener('click', registerTeacherBluetoothDevice);
        }
    }
    ...
    function openManualAttendanceModal() {
        ...
        // Populate students dropdown based on selected section
    ...
    function openBLECheckinModal() {
        // Populate sections dropdown for BLE check-in
    ...
    function registerTeacherBluetoothDevice() {
        const sectionSelect = document.getElementById('teacherBleSection');
        if (!sectionSelect) return;

        const sectionId = sectionSelect.value;
        if (!sectionId) {
            showToast('error', 'Registration Failed', 'Please select a section');
            return;
        }
        ...
        BLEService.scanForDevices()
            .then(devices => {
                ...
                return BLEService.registerCurrentDevice(sectionId);
            })
            .then(() => {
                showToast('success', 'Device Registered', 'Bluetooth device registered for section');
            })
            .catch(error => {
                showToast('error', 'Registration Failed', error.message);
            });
    ...
    function loadStudentDashboard() {
        ...
        // Update student sections carousel
    ...
    function loadReportsData() {
        // This would be implemented with charts and detailed reports
    ...
    function createSection() {
        ...
        // Get current location for the section
        LocationService.getCurrentLocation()
            .then(location => {
                const sectionData = {
                    name,
        ...
        LocationService.getCurrentLocation()
            .then(location => {
                ...
                if (state.maps.sectionLocation) {
                    ...
                    // Clear existing features and add new marker
    ...
    function loadSections() {
        ...
        if (state.userRole === 'teacher') {
            // Teacher loads their own sections
            db.collection('sections')
                ...
                .onSnapshot((snapshot) => {
                    ...
                    // Also update students when sections change
                    loadStudents();
            ...
            // Student loads sections they're enrolled in
            db.collection('sections')
    ...
    function loadStudents() {
        ...
        // For teachers, load all students in their sections
        ...
        // Load student data for all student IDs
    ...
    function loadAttendanceData() {
        ...
        if (state.userRole === 'teacher') {
            // Teacher loads attendance for their sections
            ...
            // Query attendance for all sections
            ...
            // Student loads their own attendance
            db.collection('attendance')
